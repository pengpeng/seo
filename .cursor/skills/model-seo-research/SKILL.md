...
5. **给每个词定角色**（主词 / 次级词 / GEO 前哨，标准见 [reference/keyword-selection-standard.md](/Users/pengpeng/seo/reference/keyword-selection-standard.md)）：主词软参考线 US 月量 ≥ ~100（簇合计量/意图可上提）；引擎组合词、`run <model> locally` 等常作次级；新模型近零量词多为 GEO。
...
报告**只对词下判断，不聚簇、不谈文章**——聚簇成文章是下游 seo-content 的事（且可跨报告，见 [reference/keyword-selection-standard.md](/Users/pengpeng/seo/reference/keyword-selection-standard.md) Layer 2）。产出一张 **Top 关键词（含角色判断）** 词表，字段同 brand-seo-research：**关键词 / 月量 / KD / CPC / 意图 / 角色（主词候选·次级·GEO）/ 一句话判断（含 Olares 角度）**。模型场景常见：引擎组合词（`<model> ollama/comfyui/vllm`）多作次级、新模型近零量词多作 GEO、对比替代词（`open source <闭源> alternative`）常作主词候选。
