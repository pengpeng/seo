...
产出**关键词簇**（1 簇 = 主词 + 次级 + 问题词 = 1 篇文章），字段与簇打分同 brand-seo-research（见 [reference/keyword-selection-standard.md](/Users/pengpeng/seo/reference/keyword-selection-standard.md)）：主词/次级词/问题词/簇合计量/主词 KD/评分/文体/一句话方向。模型场景常见簇：引擎组合词族（`<model> ollama/comfyui/vllm`）、本地部署族（`run <model> locally` + `<model> vram/gguf`）、对比替代族（`open source <闭源> alternative`）。
