...
|---|--------------|--------|--------|---------|------|------|-----------|
| 1 | wtr max（320/KD5）| aoostar wtr max（1,000/20）、aoostar wtr pro（480/15）、aoostar wtr max nas（50/16）| WTR Max 能跑本地大模型吗？WTR Max 有独显吗？| ~1,850 | ⭐⭐⭐ | alternative | 轴1：WTR Max 弱 iGPU 跑不动本地大模型/ComfyUI/视频，对比 Olares One 24GB CUDA 独显实测；KD5、CPC$4.11 购买意向极强 |
| 2 | oculink egpu（1,000/KD18）| egpu（9,900/42，品类背景）| 给 NAS 接 eGPU 值不值？OCuLink 能给 NAS 加显卡吗？| ~1,000 | ⭐⭐⭐ | versus | 轴2：OCuLink 补卡 TCO vs Olares One 开箱即跑本地 AI；已买 WTR Max 走信息 B 后接 GPU |
| 3 | mini pc linux（320/KD8）| mini pc server home（170/8）、homelab mini pc（70/14）、homelab nas（—/20）| 怎么在 Aoostar WTR Max 上装 Olares？mini PC 家庭服务器该装什么系统？| ~560 | ⭐⭐⭐ | alternative | 信息 B：WTR Max（AMD 版）装 Olares 教程（ROCm + OCuLink eGPU）；家庭服务器要不要能跑 AI |
| 4 | aoostar nas（390/KD28）| mini pc nas（480/27）、ai nas（~200）、best mini pc nas（—/10）、11 bay nas（—）| 最好的 AI NAS mini PC 是哪个？| ~1,070 | ⭐⭐ | listicle | 轴1：NAS/AI NAS 横评——WTR Max 存储侧、Olares One 本地 AI 侧（引实测数字）|
| 5 | egpu nas（0/KD0）| oculink nas（0/0）| NAS 能靠 eGPU 跑 AI 吗？| ~0 | ⭐ | versus | GEO 前哨："NAS 靠 eGPU 跑 AI"场景先占 AI Overview，落 Olares 对比 |
