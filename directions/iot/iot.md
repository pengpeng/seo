...
| Sonos Voice Control | Sonos | 音乐 / 系统控制，隐私向，可与 Alexa 并存 | HA Voice PE | ⬜ |
...
| Aqara G410（本地门铃冠军） | 绿米联创 | 2K + 端侧人脸 + microSD/NAS/RTSP→HA | **Aqara G410（本地 + RTSP→Frigate）** | ⬜ |
...
| TP-Link Kasa / Tapo 灯 | TP-Link | 生态平价灯泡/灯带 | ESPHome / Tasmota | ⬜ |
| DIY 像素灯带（WS2812B 等） | 开源 | WLED ~18.2k stars 事实标准 | **WLED / ESPHome（纯本地）** | ⬜ |
...
| Eve Energy | Eve Systems | Apple/Thread 隐私向、本地存储无订阅 | HA Matter Server 本地 | ⬜ |
| Belkin Wemo（反面教训） | Belkin | 2026-01-31 云关停、非 Thread/HomeKit 型号变哑 | 换 Shelly / Zigbee 插座 | ⬜ |
...
| Sensibo / Cielo | Sensibo / Cielo | 空调 IR 细分（云 API 导向） | Broadlink RM4 / ESPHome IR / SmartIR | ⬜ |
...
| 产品 / 平台 | 母公司 | 市场地位 / 融资 | Olares 平替（开源） | 报告 |
|------|------|---------|------------|------|
| SmartThings | Samsung | 平台级中枢；2025-12 430M 注册、Matter 1.4（50 设备类型） | **HA on Olares（唯一本地 Hub）** | ⬜ |
| Aqara Hub M2 / M3 / M100 | 绿米 | 多协议 Hub（Zigbee+Thread+Matter+IR）、海外线 | HA + Zigbee2MQTT | ⬜ |
| Philips Hue Bridge | Signify | Zigbee 本地桥（2023 起强制账号） | ZHA / Zigbee2MQTT 直连绕桥 | ⬜ |
| Thread Border Router（Echo/HomePod/Nest Hub/eero） | Amazon / Apple / Google | 隐形 TBR（随生态附赠，凭据托管厂商云） | HA Matter Server + 自建 OTBR | ⬜ |
| Z-Wave 800 / Long Range（Zooz 等） | Silicon Labs / Zooz | 协议阵营（2026-01 已 125 款 ZWLR 认证） | Z-Wave JS + USB 棒 | ⬜ |
| DIY 传感器（门窗/PIR/mmWave/温湿度/水浸） | 开源 | ESPHome 自制 | **ESPHome / Zigbee2MQTT（本地）** | ⬜ |
...
| SharkNinja | SharkNinja（NYSE:SN） | FY2025 高增长多品类 | 选无摄像头型号 | ⬜ |
| Eufy | Anker | HomeBase 本地 AI（隐私信誉受损） | Valetudo / 防火墙断网 | ⬜ |
...
| 产品 / 平台 | 母公司 | 市场地位 / 融资 | Olares 平替（开源） | 报告 |
|------|------|---------|------------|------|
| Bosch / Siemens Home Connect | BSH（FY2025 €15B） | **唯一可真本地**（hcpy / TLS-PSK） | **hcpy 本地 + HA** | ⬜ |
| LG ThinQ | LG | 白电大厂生态、无本地 API | HA 有限集成（经云）/ ESPHome 外接传感器 | ⬜ |
| Samsung（SmartThings 家电） | Samsung | Family Hub 冰箱（2025-11 上广告争议） | HA 有限集成（经云） | ⬜ |
| Whirlpool | Whirlpool（NYSE:WHR，$15.5B） | 北美白电巨头、无本地 API | ESPHome 外接功率/门磁 | ⬜ |
| GE Appliances | Haier 旗下 | 北美 #1 | ESPHome 外接传感器 | ⬜ |
| Miele | Miele（€5.16B） | 高端 | HA 有限集成 | ⬜ |
...
| Google TV | Alphabet | 全球 Android TV ~42%[u]；广告画像 | Jellyfin + Apple TV | ⬜ |
| Vizio SmartCast | Walmart（$23 亿收购） | **FTC 2017 ACR $2.2M**；CastOS 2029 拟北美 #1 | Jellyfin | ⬜ |
| Apple TV 4K（无 ACR 例外） | Apple | 无 ACR，第三方须 ATT 授权 | **外接 Apple TV 4K 作唯一智能源** | ⬜ |
...
| 白牌（TRENDnet 类） | 多 OEM | FTC 2013 ~700 路公开；被黑语音骚扰 | Frigate + RTSP + VLAN 隔离 | ⬜ |
...
| 产品 / 平台 | 母公司 | 市场地位 / 融资 | Olares 平替（开源） | 报告 |
|------|------|---------|------------|------|
| 儿童 GPS 表（Garmin Bounce / Xplora / TickTalk） | Garmin / Xplora 等 | 碎片化（Garmin Bounce ~12.4%[u]）；STALK 研究多款可远程监听 | Traccar 自托管定位 + 可控 tracker | ⬜ |
| 老人告警（Medical Guardian / Bay Alarm / Life Alert） | 告警 SaaS | 订阅制、持续采集位置 | HA + mmWave 跌倒（非医疗级） | ⬜ |
| Apple Watch Fall Detection | Apple | 跌倒/GPS/Medical ID 传 Apple & 911 | HA + mmWave（折中） | ⬜ |
| 物品 / 人追踪（AirTag / Tile / Jiobit） | Apple / Life360 | AirTag smart-tag ~69% 统治 | Traccar / 自选 GPS | ⬜ |
...
| 产品 / 平台 | 母公司 | 市场地位 / 融资 | Olares 平替（开源） | 报告 |
|------|------|---------|------------|------|
| Tesla | Tesla（NASDAQ:TSLA） | **员工传阅舱内/哨兵视频（Reuters）** | 无完整平替；关舱内摄像头、本地 Dashcam 无云 | ⬜ |
| GM OnStar | GM | **→LexisNexis 驾驶数据影响保费**；2024-03 停止共享 | 拒绝驾驶评分 app；Traccar 自管 | ⬜ |
| Ford / Toyota-Lexus / Hyundai-Kia / VW | 各 OEM | **Mozilla 评汽车最差隐私（25 品牌全不及格、76% 可出售）** | 无完整平替；Traccar 车队自管 | ⬜ |
| 后装 OBD（Bouncie / Vyncs） | Bouncie / Vyncs | 位置/行程长期存云、与 UBI 保险打通 | Traccar + 自购 GPS | ⬜ |
...
| Enphase IQ | Enphase（NASDAQ:ENPH） | 微逆龙头 Q1 2026 $282.9M；IQ Gateway 上报云 | HA 集成本地读数 | ⬜ |
| 智能电表 / AMI | Landis+Gyr / Itron / 公用事业 | **NILM 推断在家行为/再识别**；用户不可自托管 | 旁挂 CT 监测替代；仅读聚合数据 | ⬜ |
...
| Brilliant Labs Frame | Brilliant Labs | **唯一开源硬件方向（MIT）** | **Brilliant Labs Frame（开源）+ 本地转写** | ⬜ |
