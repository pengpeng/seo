...
- **格局借势（新）**：除产品竞品外，隐私"格局"本身是内容富矿——见 [privacy/landscape.md](/Users/pengpeng/seo/directions/privacy/landscape.md)（7 大类/约 39 子方向：合规立法、认证标准、重大泄露、AI concern、巨头监控、用户诉求，以及**企业自部署/合规采购**——衔接 [场景 8](/Users/pengpeng/seo/scenarios/scenarios.md) 的高 CPC 合规/主权/私有 AI 词）。高机会子方向：`data sovereignty`/`own your data`（品牌绑定）、`data localization`/`cross border data transfer`（本地部署不产生跨境传输）、`ChatGPT training data opt out`/`local LLM privacy`（本地推理无第三方训练）、`degoogle`/`Gmail alternative`（承接 €6.11B GDPR 罚单 + 巨头 concern 叙事）。**诚实边界**：本地部署解决"数据在哪/归谁"，但合规是流程、认证是组织行为、自托管非零风险。

**关联**：500 词分类 11（隐私/GDPR 40）、分类 14 借力大词（隐私类）；隐私竞品清单与报告见 [privacy/services.md](/Users/pengpeng/seo/directions/privacy/services.md) 与 [privacy/reports/](/Users/pengpeng/seo/directions/privacy/reports)（tuta、simplelogin、cryptpad、internxt、tresorit、gdpr、duckduckgo、startpage、brave、privacy-keywords）；**隐私格局分类总图 + 6 份 deep-research** 见 [privacy/landscape.md](/Users/pengpeng/seo/directions/privacy/landscape.md) 与 [privacy/research/](/Users/pengpeng/seo/directions/privacy/research)；商业隐私 SaaS 见 [commerce/reports/](/Users/pengpeng/seo/directions/commerce/reports)（proton、bitwarden、1password、keepassxc、vaultwarden）。
...
**分类地图**：方向 6 已建成一张**扁平 11 大类**的分类总表（IaaS/PaaS 只作行文视角）——计算/编排、存储、网络接入、数据库中间件、消息/流、**可观测**（独立大类：采集协议/指标/日志/追踪/可视化/一体化 APM）、平台中间件（Serverless/工作流/密钥）、企业·大数据、身份/安全、CI-CD、AI 基础设施，**每个项目一行**给「介绍/许可/其它/公有云替代/Olares 状态/SEO 报告」——来自 CNCF landscape × AWS/Azure/GCP 服务 × AI-infra 栈 × KubeBlocks 收录引擎的 deep-research 归一。**推理 API/向量库/Embedding/数据库中间件已从 commerce 迁入此处**（GPU 算力云仍留 commerce）。**中间件锚点**：Olares 平台层中间件由 **KubeBlocks operator** 托管、`OlaresManifest.yaml` 声明式依赖、自动注入凭据（共享 PostgreSQL 16 兼全文检索+向量、KVRocks 兼 Redis、NATS 作 MQ）——这是最硬的技术差异点。KB 标记已按[官方 supported-addons](https://kubeblocks.io/docs/preview/user_docs/overview/supported-addons) 校正。新增子类：网络拆 Mesh VPN／反向代理、密钥/HSM 与授权 ReBAC 归入身份/安全（Olares 走 OPA、非 ReBAC）、CI-CD 用 Zot 镜像仓库、AI 补 Agent 沙盒与 Agent 记忆（Mem0/Zep/Letta/Cognee）并拆开可观测评估与微调。

**洞见**：
- **headscale（2,900/mo，KD=33，CPC=$6.70）是最大机会**：用户在为 Tailscale 替代付费搜索，Olares 内置 Headscale 是最直接的免费答案；`komga tailscale`（1,900/KD=4）这类"某应用 + 远程访问"的组合词几乎零竞争，一篇"在 Olares 上用 Headscale 访问 X"即可承接。隧道/接入是 Olares 相对公有云的**独特能力**（公有云无 1:1 等价）。
...
- **AI-infra 缺口可优先补内容**：向量库（Qdrant，pgvector 兜底）、Embedding 服务化、LLM 可观测评估（Langfuse）、**Agent 沙盒（E2B/microsandbox）、Agent 记忆（Mem0/Zep/Letta/Cognee）**——是 Olares 相对薄弱、对 Personal Agent 叙事最值得补的环节。
- **非 AI 缺口按长尾/进阶对待**：Service Mesh、Serverless、可观测追踪/日志全链(Jaeger/Loki/Tempo)、GitOps(ArgoCD)、运行时安全(Falco)、授权 ReBAC、HSM——多属进阶/企业运维长尾，列此存目、按需产内容。
- **KubeBlocks 是中间件层可做的主题**：`KubeBlocks alternative`、`self-hosted DBaaS`、`database operator` 及各引擎 `X on Olares`——"一个 operator 管所有数据库 + 声明式依赖 + 自动注入凭据"是有故事的技术点。
- **企业·大数据不作重点**：分布式 SQL/OLAP/查询引擎/大数据流处理默认多节点，个人云错位（唯 ClickHouse 单机个别可用，更轻用 DuckDB）——列此存目，不投内容。
- **纪律**：这些是**长尾/技术文**，不进品牌首屏与 campaign（口径见 [basic/olares.md](/Users/pengpeng/seo/basic/olares.md) 品牌口径与写作规范）。

**关联**：分类总表见 [tech/tech-stack.md](/Users/pengpeng/seo/directions/tech/tech-stack.md)，底稿见 [tech/research/](/Users/pengpeng/seo/directions/tech/research)（索引 [README](/Users/pengpeng/seo/directions/tech/research/README.md)：middleware-taxonomy〈IaaS/PaaS 逐项目〉、cloud-infra-taxonomy、ai-infra），报告见 [tech/reports/](/Users/pengpeng/seo/directions/tech/reports)（按 11 大类子目录：headscale→03-network/mesh-vpn、frp→03-network/reverse-proxy）；商业方 Tailscale / Ngrok 归 [commerce/reports/](/Users/pengpeng/seo/directions/commerce/reports)（tailscale、ngrok）。
