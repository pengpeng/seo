...
| **App development** | Lovable, Cursor, Replit, Devin | **Cursor ~$4B ARR, Lovable ~$500M ARR** ($6.6B val), **Replit ~$265M ARR, Devin ~$492M run-rate** | Local LLM + Agent Skills (e.g. `olares-chart`): one prompt packages the image and deploys to your own Olares — no Kubernetes | Cloud tools meter you by credit/token, so heavy iteration keeps getting pricier and middlemen add margin. On Olares you run and rewrite as much as you want at no extra cost, your code never leaves the box, and you ship to your own domain. |
...
| **应用开发** | Lovable、Cursor、Replit、Devin | **Cursor ~$4B ARR、Lovable ~$500M ARR**（估值 $6.6B）、**Replit ~$265M ARR、Devin ~$492M run-rate** | 本地 LLM + Agent Skills（如 `olares-chart`）：一句话打包镜像、部署到自己的 Olares，不用懂 K8s | 云端按 credit/token 计费，越改越贵、还被中间商加价；本地怎么改怎么跑都不额外收费，代码不出本机、部署到自己域名。 |
...
- **Claude Code / Codex 归类**：本页按"操作整机的通用 Agent"归到 Personal Agent；它们同时也是编码 Agent，别和应用开发行重复陈列。
