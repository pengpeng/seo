# Slide 6 逐字稿 · Olares 五层架构

这一页把 Olares 的产品交付与系统底座放在一起看：左边是用户实际拿到的一站式解决方案，右边是支撑它的五层 OS 架构。
...
第四层是 Built-in apps，对应 Android System Apps 的角色。Desktop、Files、Market、Settings、Dashboard、Control Hub、Vault 和 Wise，把文件、密码、知识、系统管理与监控做成开箱即用的用户体验。

第五层是 Community apps，对应 Android 第三方应用和应用分发生态的角色。Olares Market 已提供 200+ 应用，覆盖 AI、生产力、媒体与智能家居等类别，用户可以一键安装；底层则继续由同一套 Framework、PaaS 和 IaaS 治理。

Open source 不是额外的第六层，而是贯穿全部五层的属性。Olares 核心仓库采用 AGPL-3.0；各层使用的第三方组件继续遵循各自的开源许可证。

这套五层架构，最终作为一套一站式产品交付给用户，也就是左栏这几件东西：Olares OS 就是右侧的整套系统；Clients 提供桌面、移动端和浏览器扩展的访问入口；Olares Space 负责域名、网络、远程访问和备份；Olares Market 让 200+ 应用一键安装；Olares CLI & Skills 则让用户用自然语言配合 agent 管理整个系统。左边是用户实际拿到手的"交付物"，右边讲的是支撑这些产品的 OS "怎么构成"。
